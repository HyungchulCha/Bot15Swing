...
class Bot15Swing():
    ...
    def stock_order(self):
        ...
        for code in self.b_l:
            ...
            if tn != tn_0900:
                ...
                chk_vol = sum([float(min_lst[i]['cntg_vol']) for i in range(15)])
    ...
    def market_to_excel(self, rebalance=False, filter=False):
        ...
        if tn > tn_093000:
            ...
            df_befor = load_xlsx(FILE_URL_DATA_15M).sort_index(axis=1)
    ...
    while True:
        ...
        try:
            ...
            if B15.init_marketday == 'Y':
                ...
                if t_n >= t_090000 and t_n <= t_153000 and B15.bool_stockorder == False:
                    B15.stock_order()
