...
dir = os.getcwd()
flist = os.listdir(dir + '/_back_15m')
...
for x in np.nditer(xlsx_list):
    ...
    if not (code in kosdaq):
        code_df = pd.read_excel(dir + '/_back_15m/' + str(x))
        code_df = code_df[::-1]
        ...
        for i, row in temp_df.iterrows():
            ...
            # has_buy == True\
            # :
            #     if buy_p * 0.94 > row['close']:
            #         sel_close = row['close']
            #         has_buy = False
            #         _ror = B15.ror(buy_p, sel_close, _ror)
...
prft_df.to_excel(dir + '/_back_result/Bot15mBacktest' + datetime.datetime.now().strftime('%m%d%H%M%S') + '.xlsx')